...
walk(root);
const importRegex = /import\s+(.*?)\s+from\s+['"](\.\.\/.*?|\.\/.*?)(?:\.jsx?|\.js|\/index(?:\.js|\.jsx))?['"]/g;
...
function resolveImport(file, imp) {
  ...
  let target = path.resolve(base, imp);
  ...
  for (const match of src.matchAll(importRegex)) {
    ...
    if (spec.startsWith('{')) {
      ...
      for (const name of names) {
        if (name && !exp.names.includes(name) && !exp.defs.includes(name)) issues.push({ file, target, type: 'named', name, exp });
      }
    } else if (!spec.startsWith('*')) {
      const name = spec.split(' as ')[0].trim();
      if (name !== 'React' && !exp.defs.includes(name)) issues.push({ file, target, type: 'default', name, exp });
    }
  }
}
if (issues.length === 0) {
  console.log('No apparent default/named import mismatches found.');
} else {
